...
**Last updated:** 2026-07-29 (end of Phase 2)
...
| 8 | TLC trip records ingested | 1 → 6a | Bronze + Silver | **In progress** | Profiled locally, `notebooks/01_profile_bronze.ipynb`; Silver built and validated locally, `notebooks/02_silver_build.ipynb`, 7 invariants passing. Metadata-driven pipeline, control table, ForEach and watermark deferred to 6a — **ADR-002** |
| 9 | Zone lookup ingested | 2 → 3 | `dim_zone` (SCD2) | **Deferred** | Raw file landed and referential integrity verified (0 orphans). SCD2 build belongs with the Gold star schema — deferred to Phase 3 — **ADR-002** |
| 10 | Weather ingested | 3 → 6a | `silver_weather_hourly` | Not started | Open-Meteo, not yet pulled |
...
| 13 | All artefacts version-controlled | 0-9 | This repo | **In progress** | Public repo, 10+ documents, 2 notebooks, 1 SQL file, 4 ADRs committed |
...
| 16 | Total budget under $60 | 0-9 | Cost tracking | **On track** | $0 spent to end of Phase 2 — **ADR-002** |
...
Phases 1-2 and are tracked here so they cannot quietly disappear.

| # | Commitment | Phase | Status | Evidence / ADR |
|---|---|---|---|---|
| 17 | Nothing silently dropped: every row flagged or quarantined | 2 | **Complete** | `notebooks/02_silver_build.ipynb`, 7 invariants passing — **ADR-003** |
| 18 | Per-month flag counts emitted to `dq_results` every run | 2 | **Complete** | `notebooks/02_silver_build.ipynb`, `dq_results` table |
| 19 | Partition by pickup date, never by filename | 2 | **Complete** | `notebooks/02_silver_build.ipynb`, invariant 6 passing |
...
| 23 | Collapse exact duplicates, retain refund pairs | 2 | **Complete** | `notebooks/02_silver_build.ipynb` — **ADR-004** |
| 24 | Money stored as DECIMAL, not DOUBLE | 2 | **Complete** | `sql/silver_build.sql` type contract |
...
| dim_zone SCD2 build | 2 | 3 | Belongs with the Gold star schema | ADR-002 |
